...
export class TaskService {
	 ...
	 */
	async read(id: string): Promise<Result<Task, TaskError>> {
	 ...
	 */
	async list(): Promise<Result<Task[], TaskError>> {
	...
	async create(params: {
		...
		dependencies?: string[];
		prd?: string;
	}): Promise<Result<Task, TaskError | PRDError>> {
	 ...
	 */
	async resolve(id: string): Promise<Result<ResolveResult, TaskError>> {
		const readResult = await this.store.readTask(id);
		if (!readResult.ok) {
		...
		const writeResult = await this.store.updateTask(task);
		if (!writeResult.ok) {
			return writeResult;
		}

		// Rebuild the index to reflect the updated state
		const listResult = await this.store.listTasks();
		if (!listResult.ok) {
		...
		for (const depId of this.#index.getDependents(id)) {
			const depTaskResult = await this.store.readTask(depId);
	...
	async delete(id: string): Promise<Result<void, TaskError>> {
		...
		const deleteResult = await this.store.deleteTask(id);
		if (!deleteResult.ok) {
